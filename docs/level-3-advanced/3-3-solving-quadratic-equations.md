...
**2.** The equation $x^2 - 3x - 10 = 0$ has two solutions. What is the greater of the two? *(student-produced response)*
...
**6.** The equation $3x^2 + 5x - 2 = 0$ has two solutions. What is the positive solution? *(student-produced response)*
...
??? success "Show full solutions"
    ...
    **2.** There is a linear term, and the coefficients are small, so try
    ...
    $-3$: those are $-5$ and $2$. So $x^2 - 3x - 10 = (x-5)(x+2)$. Setting
    each factor to zero gives $x = 5$ or $x = -2$. The greater solution is
    $x = 5$.
